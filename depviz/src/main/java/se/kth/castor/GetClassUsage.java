...
public class GetClassUsage {
	...
	static String getApiUsages = "SELECT m.libraryid, m.package, m.class, COUNT(DISTINCT(u.clientid)) " +
			"as usages FROM api_member as m JOIN api_usage as u ON m.id=u.apimemberid WHERE m.libraryid=? GROUP BY m.libraryid, m.package, m.class;";

	public static void main(String[] args) throws SQLException {

		File f = new File ("api_class_usage.csv");
		try {
			FileUtils.write(f,"library,package,class,usages,clients\n", Charset.defaultCharset());
		} catch (IOException e) {
			e.printStackTrace();
		}
